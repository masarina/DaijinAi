...
class QRCodePositionDetectionPlayer(SuperPlayer):
    ...
    def fill_position_detection_patterns(self):
        ...
        # 右上のパターン (7×7)
        grid[0:6, -7:] = -1  # 7×7の領域を-1で塗りつぶす
        grid[1:5, -6:-2] = -2  # 中央を5×5で-2に戻す
        grid[2:4, -5:-3] = -1  # 中央の3×3を-1で塗りつぶす
        grid[0:7, -8] = -2  # 左辺1マスを-2で塗りつぶす
        grid[7, -8:-1] = -2  # 下辺1マスを-2で塗りつぶす
    
        # 左下のパターン (7×7)
        grid[-7:, 0:6] = -1  # 7×7の領域を-1で塗りつぶす
        grid[-6:-2, 1:5] = -2  # 中央を5×5で-2に戻す
        grid[-5:-3, 2:4] = -1  # 中央の3×3を-1で塗りつぶす
        grid[-8, 0:7] = -2  # 右辺1マスを-2で塗りつぶす
        grid[-8:-1, 7] = -2  # 上辺1マスを-2で塗りつぶす
