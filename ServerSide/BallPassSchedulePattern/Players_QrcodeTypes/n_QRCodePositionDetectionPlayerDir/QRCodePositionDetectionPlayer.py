...
class QRCodePositionDetectionPlayer(SuperPlayer):
    ...
    def fill_position_detection_patterns(self):
        ...
        grid[2:5, 2:5] = -1  # さらに中央の3×3を-1で塗りつぶす
        grid[0:8, 0:8] = -2  # 周囲1マスを-2で塗りつぶす
        grid[0:7, 0:7] = -1  # 7x7領域を元に戻す

        # 右上のパターン (7×7)
        grid[0:7, -7:] = -1
        grid[1:6, -6:-1] = -2  # 中央を5×5で-2に戻す
        grid[2:5, -5:-2] = -1  # さらに中央の3×3を-1で塗りつぶす
        grid[0:8, -8:] = -2  # 周囲1マスを-2で塗りつぶす
        grid[0:7, -7:] = -1  # 7x7領域を元に戻す
